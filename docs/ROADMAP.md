...
| **P-41** | **Identité de classe** — Puissance orientée par la classe, `statRules`, 9 passifs sélectionnables, récompenses data-driven · **4 lots (A → D)**, **A, B, C et la partie 1 du lot D livrés ; seule la partie 2 du lot D reste** *(voir §4)* · [spec](superpowers/specs/2026-08-07-s2-identite-de-classe-design.md), révisée les 2026-09-16 et 2026-09-17 | *à chiffrer par lot* | ★★★★☆ | 🔥🔥🔥 |
...
| **P-41 D** | **Dernier lot de P-41, à moitié livré.** **Partie 1, le tutoriel — ✅ fusionnée dans `main` le 2026-09-20 (PR #44**, merge `54c28dd`, 9 commits, `30d48dc` → `639a222`, branche `feat/p41-lot-d-tutoriel`, 1159 tests) : l'étape de choix de classe déplie les passifs que `availablePassivesFor` ouvre à la classe, **dans le `ClassPassiveList` de l'écran de sélection** et non une seconde implémentation (spec §1.4) — retaper la classe déjà choisie replie sa carte au lieu de reposer son passif ; l'étape « Armure & Dégâts » fait passer son gain de démonstration par `StatGains.apply` et les `statRules` de la classe, **en deux temps** (le gain, puis le coup), sous un titre **généré** depuis la règle qui vise l'Armure et la règle écrite en clair sous les panneaux ; l'étape « Jouer des cartes » **mesure le gain réel** de part et d'autre de `playCard` au lieu de lire la valeur imprimée sur la carte. Les deux acquis que le lot B avait livrés sans les verrouiller — `critChance` forcé à 0, conversion d'armure appliquée par `playCard` — passent sous test. **Aucun mécanisme nouveau, donc aucun ADR** : la livraison applique ADR-081, ADR-090 et ADR-097 · [plan](superpowers/plans/2026-09-20-p41-lot-d-partie-1-tutoriel.md). **Partie 2, la console (spec §9.2) — 📋 planifiée le 2026-09-20, non exécutée** : `statRules` validé par l'éditeur (`"mode": "convrt"` passe encore), la création guidée de classe garantit un passif disponible, les récompenses de niveau deviennent la 8ᵉ catégorie éditable, et le menu de debug règle les cibles de la Puissance en affichant les règles de stat et le passif de la run · [plan](superpowers/plans/2026-09-20-p41-lot-d-partie-2-console-de-debug.md). **Les deux parties ne partagent aucun fichier de `lib/`** et s'exécutent dans n'importe quel ordre. ⚠️ **Deux points du §9 étaient déjà livrés** et les plans le consignent : les récompenses du tutoriel lisent le registre depuis le lot C, et la validation de `classes` comme liste de références — que le §9.2 annonçait « à écrire » — l'a été par P-49. **Une prémisse avait vieilli** : l'étape « Armure » n'a aucune condition de franchissement, et rien n'était bloqué pour un Berserker — c'est la fidélité pédagogique qui l'était · [ADR-099](../.obsidian_vault/_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md), « Conséquences assumées » | B, C (livrés) |
