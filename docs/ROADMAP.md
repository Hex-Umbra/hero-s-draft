...
- [x] Récompenses de passif éligibles par ce point d'accès — *P-41, lot C*, fusionné dans `main` le 2026-09-20 (PR #43), voir [ADR-099](../.obsidian_vault/_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md). ⚠️ **Livré par deux mécanismes distincts, et un seul passe par `availablePassivesFor`** : l'écran de sélection consomme le point d'accès en entier (tous les passifs rendus, sans `take(3)`), tandis que le conditionnement d'une récompense est un champ `requires` sur `LevelUpRewardData` qui lit `RunState.activePassive` — décision 3 d'ADR-099. P-13 branche donc ses déblocages derrière le point d'accès, et ses éventuelles récompenses conditionnées derrière `RewardRequirement`
...
| **P-41 B** | ✅ **Livré et fusionné dans `main` en entier.** Partie 1 le 2026-09-17 (PR #40) — la Puissance (`might`), à comportement identique. **Partie 2 le 2026-09-18 (PR #41**, merge `5086272`, 18 commits, 1021 tests, branche `feat/p41-lot-b-identite`) — orientation réelle de la Puissance par classe, conversion d'armure du Berserker (`statRules`), les **neuf passifs**, stats de départ, éclair de la Puissance. [ADR-097](../.obsidian_vault/_adr/ADR-097-puissance-unique-orientee-par-la-classe.md) · [spec, §7](superpowers/specs/2026-08-07-s2-identite-de-classe-design.md) · [plan de la partie 1](superpowers/plans/2026-09-17-p41-lot-b-partie-1-puissance.md) · [plan de la partie 2](superpowers/plans/2026-09-17-p41-lot-b-partie-2-identite-de-classe.md) | P-41 A, P-49 ; la partie 2 après la partie 1 |
| **P-41 C** | ✅ **Livré et fusionné dans `main` en entier.** **Partie 1 le 2026-09-18 (PR #42**, merge `152fbcc`, 11 commits, branche `feat/p41-lot-c-recompenses`, 1065 tests) — les huit récompenses de niveau sont devenues de la donnée (`assets/data/level_up_rewards/`, neuvième source d'entités) : `LevelUpRewardType`, le `rng.nextInt(6)` et 17 clés ARB ont disparu, la table `values` remplace les deux régimes de valeur concurrents, les descriptions sont des gabarits à trous, et le rouleau du carrousel comme la prose du tutoriel lisent le registre — **sans que le jeu change** · [plan](superpowers/plans/2026-09-18-p41-lot-c-partie-1-recompenses-data-driven.md) · [ADR-098](../.obsidian_vault/_adr/ADR-098-recompenses-de-niveau-en-donnee-et-gabarits-a-tr.md). **Partie 2 ✅ le 2026-09-20 (PR #43**, merge `2f850c4`, 20 commits de code, branche `feat/p41-lot-c-selection`, 1135 tests) — *Affinité* n'est plus tirée quand le passif actif ne déclare pas de Maîtrise (champ `requires` sur `LevelUpRewardData`, **le mécanisme et non la récompense**), `baseDamage` quitte `HeroData` et les trois `class.json` avec `sword_icon.dart`, et **l'écran de sélection laisse choisir le passif** : les **neuf passifs du lot B sont atteignables**, là où l'écran ne proposait que `passives.first`. La carte de classe montre désormais les stats de départ non nulles, ce que la Puissance de la classe renforce et sa règle de stat en clair, tous trois **générés depuis la donnée** (ADR-090) · [plan](superpowers/plans/2026-09-18-p41-lot-c-partie-2-filtre-et-ecran-de-selection.md) · [ADR-099](../.obsidian_vault/_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md) · [spec, §8](superpowers/specs/2026-08-07-s2-identite-de-classe-design.md) | B (livré) ; la partie 2 après la partie 1, dont elle étend le modèle de récompense |
| **P-41 D** | ✅ **Livré et fusionné dans `main` en entier — dernier lot de P-41, qu'il clôt.** **Partie 1, le tutoriel — le 2026-09-20 (PR #44**, merge `54c28dd`, 9 commits, `30d48dc` → `639a222`, branche `feat/p41-lot-d-tutoriel`, 1159 tests) : l'étape de choix de classe déplie les passifs que `availablePassivesFor` ouvre à la classe, **dans le `ClassPassiveList` de l'écran de sélection** et non une seconde implémentation (spec §1.4) — retaper la classe déjà choisie replie sa carte au lieu de reposer son passif ; l'étape « Armure & Dégâts » fait passer son gain de démonstration par `StatGains.apply` et les `statRules` de la classe, **en deux temps** (le gain, puis le coup), sous un titre **généré** depuis la règle qui vise l'Armure et la règle écrite en clair sous les panneaux ; l'étape « Jouer des cartes » **mesure le gain réel** de part et d'autre de `playCard` au lieu de lire la valeur imprimée sur la carte. Les deux acquis que le lot B avait livrés sans les verrouiller — `critChance` forcé à 0, conversion d'armure appliquée par `playCard` — passent sous test. **Aucun mécanisme nouveau, donc aucun ADR** : la livraison applique ADR-081, ADR-090 et ADR-097 · [plan](superpowers/plans/2026-09-20-p41-lot-d-partie-1-tutoriel.md). **Partie 2, la console (spec §9.2) — ✅ le 2026-09-20 (PR #45**, merge `d27edc9`, 5 commits, `70fea1d` → `6c1a8dd`, branche `feat/p41-lot-d-console`, **1179 tests**) : `statRules` est borné par l'éditeur — `"mode": "convrt"` est refusé — sur un vocabulaire **lu sur `StatRule`** et jamais recopié ; la création guidée d'une classe écrit un **passif de départ** dérivé de son identifiant, si bien qu'une classe créée est jouable au lieu d'offrir un choix vide ; `EntityValidator` gagne **`pendingIds`**, les identifiants que la même transaction va écrire — seam qui nomme un trou décrit dans le code depuis P-30 ; les récompenses de niveau sont la **8ᵉ catégorie éditable** ; et l'onglet Héros du menu de debug règle les cibles de la Puissance — la dernière non retirable — en affichant classe, règles de stat et passif actif dans le vocabulaire du fichier · [plan](superpowers/plans/2026-09-20-p41-lot-d-partie-2-console-de-debug.md) · [ADR-100](../.obsidian_vault/_adr/ADR-100-console-de-contenu-vocabulaire-du-moteur-et-ident.md). **Aucune entrée de patch note** — ni joueur, ni Technique : tout est élagué du build release par `kDebugMode`, une première pour le dépôt. **Les deux parties ne partagent aucun fichier de `lib/`** et s'exécutent dans n'importe quel ordre. ⚠️ **Deux points du §9 étaient déjà livrés** et les plans le consignent : les récompenses du tutoriel lisent le registre depuis le lot C, et la validation de `classes` comme liste de références — que le §9.2 annonçait « à écrire » — l'a été par P-49. **Une prémisse avait vieilli** : l'étape « Armure » n'a aucune condition de franchissement, et rien n'était bloqué pour un Berserker — c'est la fidélité pédagogique qui l'était · [ADR-099](../.obsidian_vault/_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md), « Conséquences assumées » | B, C (livrés) |

**Une question ouverte reste, laissée par le lot D partie 2 : la Maîtrise dans l'onglet Héros du menu de debug.** Le §9.2 de la spec nomme trois lectures et un réglage — les cibles de la Puissance, la classe, les règles de stat, le passif actif. `mastery` n'en est pas, et n'a donc pas été ajoutée : l'exécutant s'en est tenu au périmètre écrit plutôt que de l'élargir de son propre chef ([ADR-100](../.obsidian_vault/_adr/ADR-100-console-de-contenu-vocabulaire-du-moteur-et-ident.md), conséquences). **Or c'est la stat qui pilote tout P-49** : chaque passif déclare ce qu'un point de Maîtrise lui apporte, et aucun moyen n'existe aujourd'hui de la faire varier en cours de run depuis la console — il faut passer par la récompense *Affinité*, donc par une montée de niveau. Le champ coûterait sept lignes copiées de `luck` dans `debug_hero_tab.dart`. **À trancher par le propriétaire** : l'ajouter (élargissement délibéré du §9.2), ou acter que le réglage de la Maîtrise n'a pas sa place dans le tiroir. À traiter **avec P-13**, qui branchera ses déblocages derrière le point d'accès de P-49 et aura besoin de la même poignée, si elle n'est pas traitée avant.
...
1. ~~**Supprimer la chaîne `skills.json`**~~ — ✅ **livré** — 6 entrées de données, `SkillData`, `SkillController`, `SkillState`, les deux `executeSkill` et le champ de sauvegarde. Le système était **inatteignable** : aucun appelant de `_game.executeSkill(...)`, aucun bouton de compétence dans `lib/ui/`. Conception et conséquences — [ADR-084](../.obsidian_vault/_adr/ADR-084-suppression-de-la-chaine-de-competences-heroiques.md). ⚠️ **`applyLifestealBuff()`** a été conservée en paire : c'est la **façade** `RunController.applyLifestealBuff` (`run_controller.dart:438`) qui n'a plus d'appelant, l'implémentation (`run/player_stats_manager.dart:475`) restant appelée par elle. Elle vit dans `RunController`, pas dans le système de compétences ; ~~P-41 doit reprendre ou supprimer les deux~~ — **✅ sans objet depuis P-49** : la stratégie du passif de vol de vie appelle la façade (`passive_strategies.dart:198`), qui a donc retrouvé un appelant. Re-vérifié le 2026-09-20, à la clôture de P-41.
