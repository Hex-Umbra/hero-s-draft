...
| **P-41 D** | **Seul lot restant de P-41.** Mise à jour fonctionnelle du tutoriel et de la console de debug. Deux reports nommés par le lot C : l'**étape de choix de classe du tutoriel** ne propose toujours pas les passifs disponibles et `tutorial_fixtures.dart:58` suppose encore un passif unique (spec §9.1) ; l'**éditeur de contenu n'a pas appris `statRules`** (spec §9.2) · [ADR-099](../.obsidian_vault/_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md), « Conséquences assumées » | B, C (livrés) |
