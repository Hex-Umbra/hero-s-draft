...
| **P-41** | **Identité de classe** — Puissance orientée par la classe, `statRules`, 9 passifs sélectionnables, récompenses data-driven · **4 lots (A → D)**, **A, B et C livrés ; seul le lot D reste** *(voir §4)* · [spec](superpowers/specs/2026-08-07-s2-identite-de-classe-design.md), révisée les 2026-09-16 et 2026-09-17 | *à chiffrer par lot* | ★★★★☆ | 🔥🔥🔥 |
...
| **P-41 D** | **Seul lot restant de P-41.** 📋 **Planifié le 2026-09-20, en deux parties indépendantes** — elles ne partagent aucun fichier de `lib/`, et peuvent s'exécuter dans n'importe quel ordre ou en parallèle. **Partie 1, le tutoriel** (spec §9.1) — l'étape de choix de classe propose les passifs que la classe ouvre (`tutorial_fixtures.dart:57` suppose encore un passif unique), l'étape « Armure & Dégâts » fait passer son gain de démonstration par `StatGains.apply` au lieu d'écrire 4 Armure en dur, et le texte flottant de l'étape « Jouer des cartes » annonce le gain réel — les trois mentent aujourd'hui au parcours Berserker, seul concerné · [plan](superpowers/plans/2026-09-20-p41-lot-d-partie-1-tutoriel.md). **Partie 2, la console** (spec §9.2) — `statRules` validé par l'éditeur (`"mode": "convrt"` passe encore), la création guidée de classe garantit un passif disponible, les récompenses de niveau deviennent la 8ᵉ catégorie éditable, et le menu de debug règle les cibles de la Puissance en affichant les règles de stat et le passif de la run · [plan](superpowers/plans/2026-09-20-p41-lot-d-partie-2-console-de-debug.md). ⚠️ **Deux points du §9 sont déjà livrés** et les plans le consignent : les récompenses du tutoriel lisent le registre depuis le lot C, et la validation de `classes` comme liste de références — que le §9.2 annonçait « à écrire » — l'a été par P-49. **Une prémisse a vieilli** : l'étape « Armure » n'a aucune condition de franchissement, et rien n'est bloqué pour un Berserker — c'est la fidélité pédagogique qui l'était · [ADR-099](../.obsidian_vault/_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md), « Conséquences assumées » | B, C (livrés) |
