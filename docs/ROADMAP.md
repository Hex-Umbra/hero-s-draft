...
3. **Dix dérives documentaires** entre les fiches du vault et le code, listées en Partie III.C de l'[état des lieux](analysis_reports/05082026_etat_des_lieux_heros_et_cartes_Opus5.md) — auxquelles s'ajoute, relevé le 2026-09-05, le corpus de formation `docs/formation-heros-draft/` : **13 de ses 22 chapitres** documentent la chaîne de compétences supprimée comme du code vivant, listings à l'appui (`grep -lE 'SkillController|SkillData|SkillState|executeSkill|skillProvider|skills\.json' docs/formation-heros-draft/*.html | wc -l`). Il n'est indexé nulle part dans `docs/INDEX.md` : décider s'il devient un instantané daté ou s'il est mis à jour — dont deux entrées **de ce document** (§5, P-17 : « `Attaque Rapide` gratuite » et « Paladin 20 armure de base ») héritées sans re-mesure et fausses. C'est exactement le motif documenté au §10.4 pour le Tier D ; le Tier C n'a pas encore subi ce contrôle.
