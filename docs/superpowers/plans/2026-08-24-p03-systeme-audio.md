...
- Style de test : `group(...)` nommé, helpers privés préfixés `_` — voir `test/unit/deck_controller_test.dart`.
- **Convention de `ProviderContainer` pour les tests audio, tranchée après la revue de la tâche 1 :** créer le conteneur **dans le test qui en a besoin**, libéré par `addTearDown(container.dispose)` — et non en `setUp`/`tearDown`. Motif : `addTearDown` colocalise création et libération, ne peut pas être oublié, et n'impose pas un conteneur aux tests qui n'en utilisent pas. `deck_controller_test.dart` emploie `setUp`/`tearDown` parce que **tous** ses tests ont besoin du conteneur ; ce n'est pas le cas ici. Les deux motifs honorent l'intention : aucun conteneur fuité. Appliquer celui-ci dans toutes les tâches audio, sans osciller. Quand un `setUp` est déjà nécessaire pour autre chose (construire un `FakeAudioBackend` et un `AudioDirector`, par exemple), l'utiliser normalement — la règle porte sur le conteneur, pas sur l'interdiction de `setUp`.
- Commits fréquents, un par tâche minimum, message en français, préfixe conventionnel (`feat(audio):`, `test(audio):`, `docs(vault):`).
- **Un test qui garde une branche doit prouver qu'il la voit** — règle ajoutée après les revues des tâches 3 et 5, qui ont chacune trouvé un test passant pour la mauvaise raison. Quand un test existe pour verrouiller une garde précise (`if (!x) return;`, un repli, un saut), le supprimer mentalement ne suffit pas : **supprimer réellement la garde, constater que le test ÉCHOUE, la restaurer, constater qu'il passe**, et reporter les deux sorties. Ne jamais committer la version mutée. Un test qui passe encore sans sa garde ne teste pas cette garde — il teste un chemin voisin qui rend la garde inutile, et c'est indiscernable d'un vrai test tant qu'on ne casse pas volontairement ce qu'il surveille. S'applique aux tâches 7, 10 et 13.
