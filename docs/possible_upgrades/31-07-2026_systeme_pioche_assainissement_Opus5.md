...
5. **Malédictions ennemies** — reconstruire proprement ce que `IntentType.debuffDeck` promettait. `CardType.status` existe déjà et est correctement bloqué au jeu par `canPlayCard` (`effect_resolver.dart:101-103`), et `addCardToDiscardPile` (`deck_controller.dart:319`) est le point d'injection tout prêt. Figure déjà au backlog (`backlog_and_roadmap_report_22072026.md`, section « Diversification des Intentions »).
