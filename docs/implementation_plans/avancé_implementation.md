...
- feat: Indicateurs visuels pour les cartes à usage unique (Pouvoirs)
    - Clarification de la mécanique d'épuisement
        - Afin de rendre explicite la mécanique d'épuisement des cartes de type "Pouvoir", j'ai ajouté un badge rouge "USAGE UNIQUE" sur les `UiCard` et une mention "USAGE UNIQUE (Épuisement)" dans les tooltips détaillés. Cela permet au joueur de comprendre immédiatement qu'une carte ne reviendra pas dans sa défausse une fois utilisée pendant le combat.