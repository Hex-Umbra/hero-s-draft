...
- **State Management (Riverpod)**: Located in `lib/game/controllers/` (e.g., `RunController`, `DeckNotifier`, `CombatController`, `ShopController`). All business logic (damage calculation, mana cost, status ticks, turn phases) resides in Riverpod `Notifier`s, exposed via `NotifierProvider`.
