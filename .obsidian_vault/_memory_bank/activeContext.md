...
Suite à un retour de playtest externe indiquant que le joueur montait en puissance plus vite que les ennemis, la **cadence** de ce système de scaling vient d'être resserrée (ADR-072, branche `fix/combat_scaling`) : le palier géométrique HP/Dégâts passe de 5 à 2 actes et le déblocage de tier de 10 à 5 actes, sans toucher aux bases numériques, aux formules de budget, ni au plafond d'ennemis (ADR-071). Le travail est implémenté et testé (211/211, `dart analyze` propre) mais **pas encore mergé vers `main`** — le merge reste à faire ; le patch note joueur (v0.4.6, « Le Défi S'Intensifie ») a été rédigé, voir `assets/data/patch_notes.json`.
...
1. **Accélération de la Cadence du Scaling de Difficulté — Palier tous les 2 Actes & Déblocage de Tier tous les 5 Actes (Complété, branche `fix/combat_scaling`, pas encore mergé vers `main`, ADR-072)** :
   ...
   - **Restant à faire** : merge de la branche `fix/combat_scaling` vers `main` (patch note joueur v0.4.6 déjà rédigé par le sub-agent `patch_notes_writer` — voir `assets/data/patch_notes.json`).
