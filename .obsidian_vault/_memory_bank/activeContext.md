...
Le projet a finalisé et **mergé vers `main`** la refonte du scaling de difficulté de combat (branche `feature/combat_scaling`, PR #20 puis PR #21, 2026-07-25), corrigeant un bug de double comptage de l'Acte qui provoquait une explosion incontrôlée de la difficulté en mode endless, et complétée par un plafonnement du nombre d'ennemis générés par acte ainsi que la résolution de la dérive entre le log de debug et le calcul réel de budget (ADR-071). Le Système de Sauvegarde et Persistance de Run (Autosave, v3.2.0) est également mergé (branche `feat/save_run`, PR #19, 13+ commits).

Suite à un retour de playtest externe indiquant que le joueur montait en puissance plus vite que les ennemis, la **cadence** de ce système de scaling a ensuite été resserrée (ADR-072, branche `fix/combat_scaling`) : le palier géométrique HP/Dégâts passe de 5 à 2 actes et le déblocage de tier de 10 à 5 actes, sans toucher aux bases numériques, aux formules de budget, ni au plafond d'ennemis (ADR-071). Le travail est implémenté et testé (211/211, `dart analyze` propre) et cette branche a depuis été **mergée vers `main` via PR #22** (commit de merge `b32e9e9`, 2026-07-26), aux côtés d'une correction de la réactivité du bouton « Continuer » de `HomeScreen` au retour de pause/défaite (commit `17564b4`, ADR-073 — voir §2) ; le patch note joueur (v0.4.6, « Le Défi S'Intensifie ») a été rédigé, voir `assets/data/patch_notes.json`.

Le focus se tourne ensuite vers les étapes clés suivantes de la roadmap technique : l'intégration audio complète (`flame_audio`), le découpage modulaire des écrans complexes restants (`MapScreen` et `GameScreen`), le découplage du routage de navigation, l'amélioration de la couverture de tests, et l'ajout de contenu ennemi tier-1 pour compenser la réduction de variété désormais resserrée aux Actes 1-5 (au lieu de 1-10) induite par ADR-072.

> [!IMPORTANT]
> **Accélération de la Cadence du Scaling de Difficulté (branche `fix/combat_scaling`, mergée vers `main` via PR #22, ADR-072, suite d'ADR-070/071)** : `EncounterSystem._actBracketSize` (5→2 actes) et `_tierUnlockBracketSize` (10→5 actes) resserrent la fréquence du palier géométrique HP/Dégâts et du déblocage de tier, sans changer les bases géométriques (x1.35/x1.25), la rampe intra-palier, les formules de budget ou le plafond d'ennemis (ADR-071). Effet secondaire assumé : la fenêtre tier-1-only (Slime/Gobelin) se resserre des Actes 1-10 aux Actes 1-5. Voir `decisionLog.md` (ADR-072).

> [!IMPORTANT]
> **Suivi de la Refonte du Scaling de Difficulté (branche `feature/combat_scaling`, ADR-071, mergé PR #21)** : la synchronisation du log de debug (`math_combat.md`) avec le calcul réel (`playerCardsCount`, `+(act-1)*10`) a finalement été corrigée avant le merge final (commit `24d3148`, `EncounterSystem.calculateBudget()` comme source unique), et un plafond du nombre d'ennemis générés par acte (croissant, différencié combat normal/élite/boss, sans plafond ultime) a remplacé l'ancienne limite fixe de 10. Les formules de budget de combat (`ExpectedPower`/`BaseBudget`) et les multiplicateurs boss/élite restent inchangés. Voir `decisionLog.md` (ADR-071).
...
1. **Accélération de la Cadence du Scaling de Difficulté — Palier tous les 2 Actes & Déblocage de Tier tous les 5 Actes (Complété & Mergé vers `main` - branche `fix/combat_scaling`, PR #22, ADR-072)** :
   - **Retour de playtest traité** : un retour externe indiquait que le joueur montait en puissance plus vite que les ennemis, malgré le palier géométrique introduit par ADR-070. Décision retenue : resserrer la *cadence* du système existant (bases numériques et rampe intra-palier inchangées) plutôt que nerfer le joueur.
   - **`_actBracketSize` : 5 → 2 actes** (`EncounterSystem`) : le palier géométrique HP/Dégâts (x1.35 HP / x1.25 Dégâts par palier, rampe intra-palier +5%/acte HP et +3%/acte Dégâts, inchangés) s'applique désormais tous les 2 actes au lieu de 5. Facteur HP à l'Acte 25 : x36.64 (au lieu de x3.99 avec l'ancien palier).
   - **`_tierUnlockBracketSize` : 10 → 5 actes** : tier 2 (Squelette) débloqué dès l'Acte 6 (au lieu de l'Acte 11), tier 3 dès l'Acte 11 (au lieu de l'Acte 21). `maxTierAuthored` inchangé (3).
   - **Rien d'autre modifié** : formules de budget de combat, plafond du nombre d'ennemis (ADR-071, `getMaxEnemiesForNormalCombat/Elite/Boss`), et puissance du joueur restent strictement inchangés.
   - **Trade-off assumé** : fenêtre de contenu tier-1-only (Slime, Gobelin) resserrée des Actes 1-10 aux Actes 1-5, aggravant le backlog de contenu tier-1 déjà identifié par ADR-070 — voir §3 (backlog Contenu).
   - **Assurance Qualité** : cycle TDD en 2 tâches (commits `97c5fcb` puis `8bc1920`), suite de tests complète **211/211** au vert, `dart analyze` propre.
   - Détail complet dans `decisionLog.md` (ADR-072) ; design dans `docs/superpowers/specs/2026-07-26-difficulty-scaling-acceleration-design.md` ; plan dans `docs/superpowers/plans/2026-07-26-difficulty-scaling-acceleration.md`.
   - **Mergé vers `main`** : fusionné via PR #22 (commit de merge `b32e9e9`, 2026-07-26), aux côtés de la correction ADR-073 ci-dessous. Patch note joueur v0.4.6 « Le Défi S'Intensifie » déjà rédigé par le sub-agent `patch_notes_writer` — voir `assets/data/patch_notes.json`.

2. **Correction de la Réactivité du Bouton « Continuer » de `HomeScreen` au Retour de Pause/Défaite (Complété & Mergé vers `main` - branche `fix/combat_scaling`, PR #22, commit `17564b4`, ADR-073)** :
   - **Bug corrigé** : `HomeScreen._continueGame()` et `_startNewGame()` naviguent vers `MapScreen`/`ClassSelectionScreen` via `Navigator.push`, mais le menu pause et l'écran de mort (`GameOverScreen`) reviennent à l'accueil via `Navigator.popUntil((route) => route.isFirst)` plutôt qu'en recréant l'écran. `HomeScreen` ne se reconstruisait donc pas à son retour, et son `FutureProvider` sur `SaveService.hasSave()` n'était jamais réévalué : le bouton « Continuer » pouvait afficher un état obsolète (rester visible après une défaite ayant effacé la sauvegarde, ou rester absent après le démarrage d'une nouvelle run) jusqu'à un redémarrage complet de l'application.
   - **Correctif** : `_continueGame()` et `_startNewGame()` attendent désormais (`await`) l'issue de leur `Navigator.push` et appellent `setState(() {})` à son retour, forçant la réévaluation de l'état de sauvegarde quel que soit le chemin de retour emprunté (pop simple ou `popUntil`).
   - **Couverture de test** : nouveau fichier `test/widget/home_screen_save_test.dart` (95 lignes).
   - Détail complet dans `decisionLog.md` (ADR-073).

2. **Refonte du Scaling de Difficulté de Combat — Escalier Géométrique, Déblocage de Tier & Plafond d'Ennemis (Complété & Mergé vers `main` - branche `feature/combat_scaling`, PR #20 + PR #21, v0.4.5)** :
   ...
   - **Facteur d'Acte en Escalier Géométrique** : nouvelles fonctions statiques `getActBracket`, `getActPositionInBracket`, `getHpActFactor`, `getDamageActFactor` sur `EncounterSystem` — palier géométrique alors tous les 5 actes (x1.35 HP / x1.25 Dégâts, composé) avec rampe intra-palier douce (+5%/acte HP, +3%/acte Dégâts) qui se réinitialise à chaque palier. *(Cadence resserrée à 2 actes depuis par ADR-072 — voir point 1 ci-dessus.)*
   - **Déblocage de Tier d'Ennemi alors tous les 10 Actes** : `getUnlockedTier(act)` plafonné à `maxTierAuthored = 3`. `generateEnemiesForLevel` filtre le pool d'ennemis disponibles par tier débloqué avant la sélection par budget, avec repli sur le pool complet si le filtre viderait la sélection. *(Cadence resserrée à 5 actes depuis par ADR-072.)*
   - **Trade-off de contenu explicitement validé** : le Squelette (tier 2), auparavant accessible dès l'Acte 2 via la sélection molle par budget, n'était alors plus disponible avant l'Acte 11 (désormais l'Acte 6 depuis ADR-072). Décision de design consciente créant un item de backlog (davantage d'ennemis tier-1 nécessaires, fenêtre resserrée depuis aux Actes 1-5) — voir `progress.md`.
   - **Suivi (ADR-071) — Dérive log/calcul résolue** : le log de debug dupliquait la formule `playerPower`/`finalBudget` d'`EncounterSystem` pour l'affichage, et cette copie avait dérivé (omettait `playerCardsCount` et `+(act-1)*10`). Extraction d'un unique `EncounterSystem.calculateBudget()` utilisé à la fois par la génération réelle d'ennemis et par le log.
   - **Suivi (ADR-071) — Plafond du Nombre d'Ennemis par Acte** : `getMaxEnemiesForNormalCombat/Elite/Boss(act)` plafonnent désormais le nombre d'ennemis générés (+1/acte en combat normal, +1/2 actes en élite, +1/5 actes en boss, sans plafond ultime), remplaçant l'ancienne limite fixe de 10 et empêchant l'empilement artificiel de plusieurs ennemis tier-1 faibles pour épuiser un budget élite/boss.
   - **Assurance Qualité** : 6 commits TDD pour la phase initiale (spec → plan à 6 tâches → exécution avec revue à chaque étape) + commits de suivi (fix log/calcul, plafond d'ennemis), suite de tests complète au vert, `dart analyze` propre, revue de code de branche complète passée. Livré aux joueurs via le patch note v0.4.5 ("La Difficulté Mieux Maîtrisée").
   - Détail complet dans `decisionLog.md` (ADR-070 et ADR-071) ; specs dans `docs/superpowers/specs/2026-07-24-combat-difficulty-scaling-design.md` et `docs/superpowers/specs/2026-07-25-enemy-count-scaling-design.md` ; plans correspondants dans `docs/superpowers/plans/`.
...
2. **Forge de Fusion et Améliorations de Forge Pilotées par les Données (Complété - Version v3.1.0)** :
   ...
   - **Structure Data-Driven de la Forge** : Définition de toutes les runes de forge dans `assets/data/forge_upgrades.json` (nom, description bilingue, poids, couleur, icône, restrictions) avec modèle `ForgeUpgradeData` exposant un accesseur statique `getById`.
   - **Cumul de Runes & Non-Épuisement** : Retrait de l'exclusion `alreadyHas` dans `ForgeUpgradeDialog` et `ShopController` pour autoriser le joueur à posséder et cumuler plusieurs runes de même type (effets additionnés en combat).
