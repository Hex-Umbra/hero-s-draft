...
Le projet a récemment finalisé le sprint **Centralisation et Harmonisation (v0.1.9)**, **Transition Fluide de Tour (v0.1.8)**, **L'Éclat des Combats (v0.1.7)**, les **Ajustements du Gel et de la Forge (v0.1.6)**, le **Polissage Dimensionnel des Cartes en Menu (v0.2.03)**, la **Bordure Foil Progressif Unique (v0.2.02)**, la **Décomposition de la God Class UiCard (v0.2.01)**, le **Sprint de Refonte Esthétique des Cartes (v0.1.5)**, le **Sprint du Système de Forge v2 (v0.2.00)**, le **Sprint Map, Draft & Progression (v0.1.4)** et le **Sprint Boutique & Économie (v0.1.3)** :

- **v0.1.9 — Centralisation, Harmonisation et Refactoring (Phase 1)** : Centralisation de tous les délais de combat (hero/enemy dash, riposte, ticks de poison et buffs, résolution d'intentions) et de l'ensemble des configurations physiques et visuelles des textes flottants (tailles de police, durées d'effets, dérives et drift physique) dans le fichier `lib/game/game_constants.dart` pour supprimer tous les nombres magiques codés en dur. Intégration de l'immutabilité réelle des modèles d'état clés (`EntityStats`, `CombatState`, `EnemyInstance`) avec `@immutable` et `List.unmodifiable` pour sécuriser le flux d'état Riverpod. Création de `DamagePipeline` pour unifier et centraliser l'intégralité des calculs de dégâts physiques et magiques du jeu (calculs des cartes, compétences, intentions ennemies, et gestion propre de la faiblesse, des critiques, du choc et de la vulnérabilité).
- **v0.1.8 — Transition Fluide de Tour (Reset d'Armure)** : Réinitialisation de l'armure du joueur à 0 au début de son tour pour rééquilibrer la génération d'armure et éviter l'accumulation infinie entre les tours. Suppression visuelle des popups négatifs d'armure ("-X") et de la secousse de bouclier pendant cette phase via un drapeau de suppression temporaire sur `HeroCard` (`suppressArmorChangeAnimation`), pour éviter d'indiquer faussement que le joueur subit des dégâts lors de cette transition automatique.
- **v0.1.7 — L'Éclat des Combats** : Embellissement des nombres flottants (`FloatingText`) avec des ombres néon colorées et des symboles descriptifs (💥 pour les critiques, 🧪 pour le poison, 🛡️ pour le bouclier), rotation aléatoire de départ, et cinématique d'échelle élastique suivie d'une pulsation infinie pour les critiques. Le déclenchement visuel des critiques se base désormais sur la propagation d'état `lastActionWasCrit` calculée en phase métier (déterministe). L'impact visuel est renforcé par des tremblements plus prononcés, des flashs dorés vifs, et 35 particules physiques. La jauge rouge de catch-up des dégâts de la barre de vie se vide plus lentement (décélération à 1200ms sous `Curves.easeOut`) tandis que le soin reste instantané sur le catch-up et progressif sur le vert (500ms). **Incorpore également** : le fix du Croc Kunaï (combat-long `'armor_mastery'` StatusEffect et getter dynamique `effectiveArmorMastery`), l'animation dynamique des particules Canvas du carrousel de reliques (via `AnimationController` Flutter avec gravité, friction et fade), l'exclusion des cartes de rareté unique dans les récompenses de cartes post-boss, la refonte visuelle de l'option de clonage Magic Mirror en boutique (affichage de l'interface `UiCard` complète avec sa rareté, ses runes d'upgrades de forge, ses descriptions d'effets, son coût, et ses animations de survol avec un dialogue élargi à `maxWidth: 550` et défilement horizontal), **le caching de session de clonage** `cloneOptions` dans `ShopState` empêchant l'exploit de reroll gratuit par réouvertures de fenêtres, et la **désactivation dynamique** des boutons de services de la boutique (Reroll, Heal, Purge, Expand, Clone) si les fonds en or du joueur sont insuffisants.
...
1. **Centralisation, Harmonisation et Refactoring (Version v0.1.9)** :
...
2. **Transition Fluide de Tour & Reset d'Armure (Version v0.1.8)** :
...
2. **L'Éclat des Combats (Version v0.1.7)** :
