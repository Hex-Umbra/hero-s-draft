...
| Boutique | `ShopController` / `shopProvider` | Achat/purge/clone cartes, soin, expansion, reroll, exclusion logique des cartes de rareté unique (v0.1.3), et modal Magic Mirror amélioré avec UiCard complète et défilement horizontal (v0.1.7) |
...
| Option Magic Mirror Premium | `ShopScreen` / `_CloneCardItem` | Modal de clonage de boutique utilisant le widget `UiCard` complet avec runes d'upgrades, rareté, effets de survol/glow réactifs, défilement horizontal et largeur de dialogue étendue (`maxWidth: 550`) pour la responsivité (v0.1.7) |
