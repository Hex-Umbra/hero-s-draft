...
- **Version actuelle** : v3.4.0 — Plafonnement du Nombre d'Ennemis par Acte & Résolution de la Dérive Log/Calcul (branche `feature/combat_scaling`, mergée vers `main` via PR #20 puis PR #21). v3.2.0 (Autosave, `feat/save_run`) également mergée (PR #19).

> [!NOTE]
> **`feature/combat_scaling` mergée vers `main`** (PR #20 puis PR #21, 2026-07-25) : refonte du scaling de difficulté de combat livrée (patch note v0.4.5). Prochain sujet de brainstorming ouvert : la vitesse perçue du scaling (palier géométrique tous les 5 actes, ADR-070) est jugée à accélérer. Voir §1 (Système de Combat), backlog Contenu (§3), historique des releases (§7 — v3.3.0/v3.4.0) et `decisionLog.md` (ADR-070, ADR-071).
...
| Scaling de Difficulté en Escalier Géométrique (mergé v0.4.5) | `EncounterSystem.getHpActFactor/getDamageActFactor/getActBracket` | Corrige le double comptage de l'Acte (`enemyLevel` + terme linéaire direct). L'Acte agit désormais uniquement via un facteur géométrique par palier de 5 actes (x1.35 HP / x1.25 Dégâts par palier + rampe intra-palier douce réinitialisée à chaque palier). `enemyLevel` ne dépend plus jamais de l'Acte. Voir ADR-070. |
| Déblocage de Tier d'Ennemi tous les 10 Actes (mergé v0.4.5) | `EncounterSystem.getUnlockedTier`, `generateEnemiesForLevel` | Filtre le pool d'ennemis disponibles à `tier <= unlockedTier` (tier 2 dès Acte 11, tier 3 dès Acte 21, plafond 3) avant sélection par budget, avec repli sur le pool complet si le filtre viderait la sélection. Gating strict : le Squelette (tier 2) n'apparaît plus avant l'Acte 11 (auparavant dès l'Acte 2). |
...
- [ ] **Nouveaux ennemis tier-1** pour compenser la réduction de variété des Actes 1-10 (roster actuel : Slime, Gobelin uniquement) causée par le gating strict de tier introduit par la refonte du scaling de difficulté (branche `feature/combat_scaling`, ADR-070). Trade-off de design explicitement validé, pas un oversight — voir `decisionLog.md` (ADR-070) et `activeContext.md`.
