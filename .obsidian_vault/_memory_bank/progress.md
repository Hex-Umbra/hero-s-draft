...
|:---|:---|:---|
| Tests automatisés (jeu) | 1135 au vert | `flutter test` |
| Fichiers de test | 146 | `find test -name "*.dart" \| wc -l` |
| Analyse statique | 0 erreur (`No issues found!`) | `dart analyze` |
| Fichiers Dart (`lib/`) | 246 | `find lib -name "*.dart" \| wc -l` |
| Lignes de code (`lib/`) | 49 913 | `find lib -name "*.dart" -exec cat {} + \| wc -l` |
...
> [!NOTE]
> **Métriques ci-dessus mesurées sur `main`.** **Le chantier P-41 y est fusionné jusqu'au lot C
> inclus — seul le lot D reste.** Dernier entré : **le lot C partie 2** — le choix du passif à la
> sélection de classe et le conditionnement des récompenses — par la PR #43 (2026-09-20, merge
> `2f850c4`, 20 commits de code,
> [ADR-099](../_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md)) ; **le lot C
> partie 1**, PR #42 ([ADR-098](../_adr/ADR-098-recompenses-de-niveau-en-donnee-et-gabarits-a-tr.md)) ;
...
>
> **Note de version `0.5.2` rouverte en place cinq fois**, même numéro à chaque fois — P-49
> (`5f168db`), lot B partie 1 (`2da3c23`), partie 2 (`83e65b9`), lot C partie 1 (`bc533b7`), puis
> lot C partie 2 le 2026-09-20 (`c29487e`). **Pas encore taguée** (`docs/ROADMAP.md` §4).
