...
|:---|:---|:---|
| Tests automatisés (jeu) | 1159 au vert | `flutter test` |
| Fichiers de test | 148 | `find test -name "*.dart" \| wc -l` |
| Analyse statique | 0 erreur (`No issues found!`) | `dart analyze` |
| Fichiers Dart (`lib/`) | 246 | `find lib -name "*.dart" \| wc -l` |
| Lignes de code (`lib/`) | 50 256 | `find lib -name "*.dart" -exec cat {} + \| wc -l` |
...
> [!NOTE]
> **Métriques ci-dessus mesurées sur `main`.** **Le chantier P-41 y est fusionné jusqu'à la
> partie 1 du lot D — seule la partie 2 du lot D reste.** Dernier entré : **le lot D partie 1** —
> le tutoriel enseigne la classe choisie — par la PR #44 (2026-09-20, merge `54c28dd`, 9 commits,
> **sans ADR** : aucun mécanisme nouveau) ; **le lot C partie 2**, PR #43 ([ADR-099](../_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md)) ;
...
| Fonctionnalité | Implémentation | Détails |
|:---|:---|:---|
| Une entité, un fichier | `assets/data/` | **85 entités dans 8 catalogues** répertoriés, déclarés par **9 sources** — vérifié le 2026-09-18 ; le nom du fichier **est** l'`id` ; `audio.json` et `patch_notes.json` restent des documents de configuration |
...
verbatim sous `.obsidian_vault/_archive/` (`2026-09-16-progress-releases.md` pour la
dernière rotation, `2026-09-05-progress-releases.md` pour la précédente).
