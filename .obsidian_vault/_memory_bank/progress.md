...
| Option Magic Mirror Premium | `ShopScreen` / `_CloneCardItem` | Modal de clonage de boutique utilisant le widget `UiCard` complet avec runes d'upgrades, rareté, effets de survol/glow réactifs, défilement horizontal et largeur de dialogue étendue (`maxWidth: 550`) pour la responsivité, avec persistance `cloneOptions` anti-exploit (v0.1.7) |
