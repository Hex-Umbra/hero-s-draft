...
> [!NOTE]
> **Branche parallèle en attente de merge — `feature/combat_scaling`** : Refonte du scaling de difficulté de combat (201/201 tests au vert, `dart analyze` propre) corrigeant un double comptage de l'Acte dans `EncounterSystem`. Conçue et implémentée, **non mergée vers `main`** à ce jour (conservée en l'état par choix du mainteneur). Voir §1 (Système de Combat), backlog Contenu (§3), historique des releases (§7 — v3.3.0) et `decisionLog.md` (ADR-070).
