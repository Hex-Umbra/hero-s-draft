...
> **le lot D partie 1**, PR #44 (**sans ADR** : aucun mécanisme nouveau) ;
> **le lot C partie 2**, PR #43 ([ADR-099](../_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md)) ;
> **le lot C partie 1**, PR #42 ([ADR-098](../_adr/ADR-098-recompenses-de-niveau-en-donnee-et-gabarits-a-tr.md)) ;
> **le lot B en entier**, PR #40 puis #41 ([ADR-097](../_adr/ADR-097-puissance-unique-orientee-par-la-classe.md),
> qui amende la décision 2 d'[ADR-095](../_adr/ADR-095-passage-unique-des-gains-scission-des-puissances-et.md)) ;
> **P-49**, PR #39 ([ADR-096](../_adr/ADR-096-passifs-partages-eligibilite-et-maitrise-hybride.md)) ;
> **le lot A**, PR #38. Détail des livraisons en §4 et dans `activeContext.md`.
>
...
| Chaîne de migration de schéma | `SaveMigrator`, `saveMigrator` (`lib/services/save_migrations.dart`) | Amène un blob ancien à la version courante (2) au lieu de l'effacer ; clé `run_save`, repli en lecture sur l'ancienne `run_save_v1` — [ADR-095](../_adr/ADR-095-passage-unique-des-gains-scission-des-puissances-et.md) |
| Sauvegarde corrompue = échec total | `SaveService.load()` | JSON illisible ou `schemaVersion` absente/invalide → échec propre, pas de récupération partielle |
| Sauvegarde d'un build plus récent | `SaveFromNewerBuildException`, `SaveLoadResult.savedByNewerBuild` | Jamais effacée : conservée pour le build qui sait la lire, message dédié à l'accueil — [ADR-095](../_adr/ADR-095-passage-unique-des-gains-scission-des-puissances-et.md) |
...
amendé par [ADR-095](../_adr/ADR-095-passage-unique-des-gains-scission-des-puissances-et.md).
...
| 15 étapes interactives, choix de classe **et de son passif** en amont | `TutorialScreen`, `lib/tutorial/widgets/` | Guidage pas-à-pas depuis le choix de classe jusqu'aux reliques ; l'étape 02 déplie les passifs que la classe ouvre par `availablePassivesFor` et réutilise le `ClassPassiveList` de l'écran de sélection — jamais une seconde implémentation ; étapes 02-03 verrouillées une fois franchies ; détail complet en [`_rules/08-00`](../_rules/08-00-systeme-de-tutoriel-autonome.md) |
...
| Le tutoriel enseigne la classe choisie | `TutorialArmorWidget`, `TutorialPlayCardWidget`, `TutorialEngine.gainArmorForDemo` | Le gain d'Armure de démonstration passe par `StatGains.apply` et les `statRules` de la classe, jamais par une écriture directe ; le panneau se titre depuis la règle qui vise l'Armure (`shortTitle`) et la règle est écrite en clair sous les panneaux ; le texte flottant mesure le gain **réel** de part et d'autre de `playCard` au lieu de lire la valeur imprimée sur la carte — le parcours Berserker y voit sa Puissance, pas une Armure qu'il ne garde pas |
...
| Fonctionnalité | Implémentation | Détails |
|:---|:---|:---|
| Une entité, un fichier | `assets/data/` | **85 entités dans 8 catalogues** répertoriés, déclarés par **9 sources**, dont **8 catégories éditables** — re-compté le 2026-09-20 ; le nom du fichier **est** l'`id` ; `audio.json` et `patch_notes.json` restent des documents de configuration |
...
| Récompenses de niveau en donnée | `assets/data/level_up_rewards/`, `LevelUpRewardData` | Huit fiches — six tirables, deux mythiques, séparées par `pool` ; table `values` explicite par palier, **aucun multiplicateur en code** ; descriptions en gabarits à trous (`{amount}`, `{passive}`, `{effect}`) que le tirage, l'écran de draft, le rouleau et la prose du tutoriel remplissent — [ADR-098](../_adr/ADR-098-recompenses-de-niveau-en-donnee-et-gabarits-a-tr.md) |
...
| Tiroir de debug | `DebugDrawer`, `lib/ui/widgets/debug/tabs/` | Ancré au bord gauche : Héros/Run/Deck/Reliques sur la carte, Combat seul en combat — [ADR-088](../_adr/ADR-088-tiroir-de-debug-ancre-et-sortie-de-combat-par-sa-pr.md). L'onglet Héros règle les **cibles de la Puissance** (puces générées, la dernière non retirable) et affiche classe, règles de stat et passif actif en lecture seule, **dans le vocabulaire du fichier** — [ADR-100](../_adr/ADR-100-console-de-contenu-vocabulaire-du-moteur-et-ident.md). ⚠️ **La Maîtrise n'y figure pas** : question ouverte portée en `docs/ROADMAP.md` §4 |
| Éditeur de contenu | `ContentEditorScreen`, `lib/services/content_editor/` | Créer ou modifier une entité des **8 catégories**, classe entière en un geste ; validation totale avant écriture transactionnelle — [ADR-089](../_adr/ADR-089-editeur-de-contenu-seam-disque-et-validation-totale.md). Formulaire inféré du document, sons et images importés sous rollback — [ADR-092](../_adr/ADR-092-formulaire-infere-du-document-et-ressources-liees.md). Onglets de type, explorateur, barre d'actions fixe, contrat de choix — [ADR-093](../_adr/ADR-093-habillage-editeur-jetons-nommes-et-contrat-de-choix.md) |
| L'éditeur au niveau de P-41 | `entity_descriptor.dart`, `class_recipe.dart`, `entity_validator.dart` | `statRules` borné au vocabulaire **lu sur `StatRule`** (`"mode": "convrt"` refusé) ; la création guidée écrit un **passif de départ** dérivé de l'id de la classe, donc une classe créée est jouable ; les récompenses de niveau sont la 8ᵉ catégorie ; `pendingIds` ouvre les références à ce que la **même transaction** va écrire — [ADR-100](../_adr/ADR-100-console-de-contenu-vocabulaire-du-moteur-et-ident.md) |
...
## 4. Historique des releases (6 dernières)

| Version | Date | Titre | Description des changements clés |
|:---|:---|:---|:---|
| **0.5.2** | 2026-09-16 | Des Sauvegardes Qui Durent (P-41 lot A, puis P-49, P-41 lot B parties 1 et 2, **le lot C en entier**, et **le lot D en entier** — P-41 clos) | **Taguée et publiée le 2026-09-21** (`v0.5.2` sur `d27edc9`, tag annoté), au terme de la campagne de test manuelle du propriétaire : `release.yml` run `35626036645`, **neuf jobs verts** (`verify-version`, `quality`, `build-web`, `build-windows`, `deploy-web`, `smoke-test`, `deploy-site`, `release-windows`, `notify-discord`), release GitHub en pré-release avec `heros-draft-v0.5.2-windows.zip`. **Publiée sans attendre les cartes de P-42**, contrairement à ce que la planification retenait depuis le 2026-09-05 — celles-ci iront au numéro suivant. Aucun changement de gameplay visible : les bonus du héros distinguent désormais les cartes Attaque des cartes Compétence en interne (scission de `attaque` en `attackPower`/`skillPower`/`alterationPower`, `PowerRules`), sans effet sur les dégâts affichés. Visible : le jeu convertit désormais une sauvegarde écrite par une version précédente au lieu de la perdre (`SaveMigrator`, première chaîne de migration du projet) ; une sauvegarde écrite par une version plus récente n'est plus effacée par une version plus ancienne, elle est conservée avec un message dédié à l'écran d'accueil, et n'est plus proposée aux versions antérieures à `0.5.2` sur le site. Fusionné dans `main` par la PR #38 (`f8be03a`, 2026-09-16), branche `feat/p41-lot-a` supprimée — [ADR-095](../_adr/ADR-095-passage-unique-des-gains-scission-des-puissances-et.md). **Note rouverte en place deux fois le 2026-09-17, sans changer de numéro** : pour P-49 (`5f168db`, PR #39) — la Maîtrise d'Armure devient la Maîtrise et renforce ce que produit le passif, la Forge d'Acier devient l'Affinité, et l'Armure du Berserker applique la Maîtrise à chaque tranche de 10 PV manquants au lieu d'une seule fois, [ADR-096](../_adr/ADR-096-passifs-partages-eligibilite-et-maitrise-hybride.md) ; puis pour la partie 1 du lot B de P-41 (`2da3c23`, PR #40) — la Force et l'Attaque du héros deviennent la **Puissance** partout dans les textes et la fiche de stats dit ce qu'elle renforce, **à dégâts inchangés**, [ADR-097](../_adr/ADR-097-puissance-unique-orientee-par-la-classe.md). **Rouverte une troisième fois le 2026-09-18 pour la partie 2 du même lot** (`83e65b9`, PR #41) : c'est la première entrée de cette note que le joueur *ressent* — la Puissance du Paladin renforce tout, celle du Berserker ses seules Attaques, celle du Mage ses Compétences et ses altérations ; le Berserker ne garde plus jamais d'Armure, chaque point devient une Puissance d'un tour ; chaque classe passe de un à **trois passifs** dont seul le premier est atteignable jusqu'au lot C ; stats de départ différenciées et éclair de la Puissance. **Rouverte une quatrième fois le même jour pour la partie 1 du lot C** (PR #42, merge `152fbcc`) : une seule entrée, en Technique, les huit récompenses de montée de niveau reposant désormais sur une fiche par récompense — **à gains, libellés et probabilités identiques**, [ADR-098](../_adr/ADR-098-recompenses-de-niveau-en-donnee-et-gabarits-a-tr.md). **Rouverte une cinquième fois le 2026-09-20 pour la partie 2 du lot C** (`c29487e`, PR #43, merge `2f850c4`) — **le joueur choisit son passif à la sélection de classe**, et les neuf passifs du lot B deviennent atteignables ; l'écran perd les dégâts de base qu'il affichait (5, 15, 10) et qu'aucune run ne reprenait, et gagne les stats de départ non nulles, ce que la Puissance de la classe renforce et sa règle de stat en clair ; l'*Affinité* cesse d'être tirée quand le passif actif ne déclare aucune Maîtrise. Deux correctifs visibles y entrent : la grille de classes passe à une colonne sur mobile au lieu de comprimer ses cartes, et les boutons d'un dialogue se replient au lieu de déborder. [ADR-099](../_adr/ADR-099-choix-du-passif-et-conditionnement-des-recompenses.md). ⚠️ **Première réécriture d'une entrée existante** de cette note par `patch-notes-writer` : elle annonçait cet écran comme à venir, la livraison la rendait fausse. La note n'était alors **ni taguée ni publiée** — dernière release `v0.5.1` —, ce qui a rendu cette réécriture sans effet sur un joueur ; la publication du 2026-09-21 porte donc d'emblée le texte corrigé. **Rouverte une sixième fois le 2026-09-20 pour la partie 1 du lot D** (PR #44, merge `54c28dd`) : deux entrées, **aucune réécriture** — le tutoriel laisse choisir le passif de la classe, et ses étapes « Armure & Dégâts » et « Jouer des cartes » suivent enfin la règle de la classe choisie au lieu d'annoncer un gain d'Armure au Berserker, qui n'en garde aucun. **La partie 2 du lot D, fusionnée le 2026-09-20 (PR #45, merge `d27edc9`), n'a pas rouvert cette note et n'y figure donc pas** : elle ne touche que l'outillage — éditeur de contenu et menu de debug —, élagué du build release par `kDebugMode`, sans un fichier d'`assets/data/` ni une clé ARB modifiés. C'est la **première livraison du projet à entrer dans `main` sans aucune entrée de patch note**, Technique comprise [ADR-100](../_adr/ADR-100-console-de-contenu-vocabulaire-du-moteur-et-ident.md). |
| **0.5.1** | 2026-09-05 | Chaque Chose à Sa Place (P-48, P-40 bloc 1) | **PATCH et non MINOR** : les deux chantiers de fond n'ont aucun effet visible, et ce que le joueur voit relève du polish. Visible : le dictionnaire de cartes, le tirage du deck de départ et le tutoriel partagent désormais le même critère rareté → coût → id (`CardData.compareByDisplayOrder`) au lieu de suivre l'ordre du catalogue — **le même ordre, pas la même liste** : le dictionnaire groupe d'abord par `CardType` et ne trie qu'à l'intérieur d'un groupe, là où le pool de draft est plat ; l'onglet Reliques du dictionnaire trie désormais par `id` au sein de chaque rareté ; et la sélection de classe suit un `displayOrder` explicite (Paladin, Berserker, Mage). **Visible aussi, et subi** : le passage des ids de passifs en `snake_case` (`7da5db2`, lots 1-2) casse le passif des sauvegardes antérieures — clé et `schemaVersion` inchangés, donc la partie se recharge et perd son passif jusqu'à sa fin, `MissingSaveItem` à l'appui. Consigné nulle part avant le 2026-09-05 sinon dans le message du commit ; la note de version est le seul canal prévenant *avant*. Invisible mais réel, porté en Technique : la réorganisation des données ([ADR-085](../_adr/ADR-085-regle-de-partage-catalogue-configuration.md), [ADR-086](../_adr/ADR-086-autorite-du-repertoire-avec-expiration-de-la-toler.md)) et la suppression de la chaîne de compétences ([ADR-084](../_adr/ADR-084-suppression-de-la-chaine-de-competences-heroiques.md)). **Écarté délibérément** : le correctif de compilation d'`audioplayers` sur MSVC récents (`e56295e`) est déjà couvert par le tag `v0.5.0`, `git tag --contains` le confirme. **Troisième rattrapage manuel d'affilée** du libellé visible de la carte « actuelle » de `site/index.html`, qui porte le numéro en toutes lettres là où le skill ne rafraîchissait que les `href` — la cause est cette fois corrigée dans `patch-notes-writer` plutôt que contournée. |
...
Les releases sorties de ce tableau sont conservées verbatim sous `.obsidian_vault/_archive/` :
`2026-09-20-progress-releases-v3x.md` (les quatre `v3.x`, sorties le 2026-09-20 sur
dépassement du plafond de lignes), `2026-09-16-progress-releases.md` et
`2026-09-05-progress-releases.md` pour les deux rotations FIFO précédentes.

> [!NOTE]
> **Le schéma `v3.x` est gelé**, et il a quitté ce tableau le 2026-09-20 avec ses quatre
> dernières lignes, conservées telles quelles à l'archive. La version de référence est celle
> d'`assets/data/patch_notes.json` (`0.4.x`) depuis le 2026-08-03, maintenue avec
> `pubspec.yaml` par `patch-notes-writer` : toute ligne ajoutée ici est clée sur elle.
