...
| `cardPickup` | `CardAnimator.playAnimation()`, au tout début du geste de jeu — `lib/game/components/visual_effects/card_animator.dart:133` |
| `cardPlay` | Même méthode, juste après — résolu via `CardData` comme `AudioSource` (`sfx` propre, puis `animation`, puis défaut) — `card_animator.dart:141` |
